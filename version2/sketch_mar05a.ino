...
bool calender(int *sto1,int *sto2,int *sto3)
{
...
void short_time_fun(bool *always_on,int *y,int *m,int *d)
{
      u8g2.clearBuffer();
      ...
      while(*always_on)
      {
          ...
          datastr[--ctr]=0;
          if(datastr[0]=='e'&&datastr[1]=='x'&&datastr[2]=='i'&&datastr[3]=='t')
          {
          *always_on=false;
          u8g2.setCursor(0, 42);
          u8g2.print("Exiting...");
...
void usbing(int *y,int *m,int *d)
{
  bool has_command=true;
  bool always_on=false;
  if(has_command||always_on)
  {
      if(Serial.read()=='c'&&Serial.read()=='m'&&Serial.read()=='d')
        {
          always_on=true;
          short_time_fun(&always_on,y,m,d);
          has_command=false;
        }
      else
        {
